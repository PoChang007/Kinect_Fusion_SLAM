...
void ThreeDViewer::SetUpPointClouds(std::shared_ptr<Kinfu::KinfuPipeline> &kinect_fusion_system)
{
	...
	for (int u = 0; u < _rows; u++)
	{
		...
		{
			int index = u * _cols + v;
			if (kinect_fusion_system->_system_utility->ray_casting_data->surface_prediction_z.ptr<float>(u)[v] != 0.f)
			{
				point_cloud[0]->points[index].x = kinect_fusion_system->_system_utility->ray_casting_data->surface_prediction_x.ptr<float>(u)[v];
				point_cloud[0]->points[index].y = kinect_fusion_system->_system_utility->ray_casting_data->surface_prediction_y.ptr<float>(u)[v];
				point_cloud[0]->points[index].z = kinect_fusion_system->_system_utility->ray_casting_data->surface_prediction_z.ptr<float>(u)[v];

				std::uint8_t r((int)kinect_fusion_system->_system_utility->color_image.ptr<uchar>(u)[3 * v]);
				std::uint8_t g((int)kinect_fusion_system->_system_utility->color_image.ptr<uchar>(u)[3 * v + 1]);
				std::uint8_t b((int)kinect_fusion_system->_system_utility->color_image.ptr<uchar>(u)[3 * v + 2]);
